		...
		<section class="his-box">
			...
			<ul class="rank-list">
				<li class="rank-item" v-for="(item,index) in his" :key="index">
					...
					<div class="comic-choose" @click="chooseone(index)">
						<i class="unchoose" :class="{clickchoose:cid.indexOf(index)>=0}"></i>
					</div>
			...
			del(){
				...
				for(var i=clen;i>0;i--){
					if(this.cid.length!=0){
						this.his.splice(this.cid[0],1)
						this.cid.splice(0,1)
						console.log(this.cid)
					}else if(this.cid==[]){
						this.his.splice(this.cid[0],0)
					}
				}	
			}
		}
